...
public class CircleFlowIndicator extends View implements FlowIndicator {
	...
	protected void onDraw(Canvas canvas) {
		...
		int left = getPaddingLeft() + radius;
		int top = getPaddingTop() + radius;
		int size = 2 * radius + radius;
		for (int iLoop = 0; iLoop < count; iLoop++) {
			canvas.drawCircle(left + (iLoop * size), top, radius, mPaintStroke);
		}
		int cx = 0;
		if (flowWidth != 0) {
			// Draw the filled circle according to the current scroll
			cx = ((currentScroll) * size) / flowWidth;
		}
		// The flow width has been upadated yet. Draw the default position
		if (cx > count * ((2 * radius + radius)) - radius) {
			cx %= count * (2 * radius + radius);
		}
//		if (cx > (count - 1) * size + radius) {
//			cx %= count * size;
//			cx -= count * size;
//		}
	    canvas.drawCircle(left + cx, top, radius, mPaintFill);
